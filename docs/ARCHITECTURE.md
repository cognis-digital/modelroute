...
- **rules/analyzers** apply the heuristics shipped in `modelroute/core.py`.
- **score** ranks by severity.
- **MCP server** (`modelroute mcp`) exposes `scan` for Cognis.Studio agents.
