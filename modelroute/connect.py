"""Native cognis-connect emit for modelroute — forward findings to any platform.

Maps modelroute's JSON output to the canonical `Finding` and forwards it via
...
Usage:
    modelroute ... --format json | modelroute-emit --to stix
    modelroute-emit --to slack --url $WEBHOOK --dry-run < findings.json
"""
...
def map_record(rec: dict) -> dict:
    ...
    try:
        out = dict(rec)
        out['title'] = rec.get('name', '') + ' - ' + rec.get('model', '')
